// ... imports dan interfaces biarkan sama ...
...
export function calculateAnalytics(
  ...
  // PROTEKSI RUNTIME: Pastikan semua data fallback ke array kosong [] jika undefined dikirimkan
  orders = orders || [];
  orderItems = orderItems || [];
  menus = menus || [];
  const safeSplitBills = splitBills || [];
  ...
  const filteredSplitBills = safeSplitBills.filter(sb => {
  ...
  relevantItems.forEach(item => {
    const menu = menus.find(m => m.id === item.menu_id || m.id === item.menus_id); // Dukung menus_id & menu_id
    if (menu) {
  ...
  relevantItems.forEach(item => {
    const menu = menus.find(m => m.id === item.menu_id || m.id === item.menus_id);
    if (menu) {
  ...
  // Generate all 24 hours for complete daily view
