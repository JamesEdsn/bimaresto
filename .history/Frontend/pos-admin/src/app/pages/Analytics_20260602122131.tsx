...
const getDailyReport = async (days: number) => {
  const res = await api.get('/reports/daily', { params: { days } });
...
const Analytics = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const data = await getDailyReport(daysFromRange);
        if (!cancelled) setDailyReports(Array.isArray(data) ? data : []);
      } catch (err: any) {
        if (!cancelled) {
          console.error('Failed to load daily report:', err);
          setReportsError('Gagal mengambil data laporan harian');
          setDailyReports([]);
        }
      } finally {
  ...
  const dailyRevenue = useMemo(() => {
    // dailyReports has: {date,total_orders,total_revenue,...}
    const sorted = [...dailyReports].sort((a, b) => String(a.date).localeCompare(String(b.date)));
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-start justify-between gap-4 flex-wrap">
            ...
            <div className="text-right">
              {isLoadingReports ? (
                <p className="text-slate-500 text-[13px]">Memuat laporan...</p>
              ) : reportsError ? (
                <p className="text-red-600 text-[13px]">{reportsError}</p>
              ) : (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
            <MetricCard
              title="Total Revenue"
              value={formatCurrency(dailyRevenue.reduce((sum: number, d: any) => sum + (Number(d.revenue) || 0), 0))}
