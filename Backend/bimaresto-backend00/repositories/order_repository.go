...
type OrderRepository interface {
	...
	// UpdateOrderStatus updates the status field of an order and optionally frees the table when completed
	UpdateOrderStatus(order *models.Order, status string) error
}
...
func (r *orderRepository) FindAll() ([]models.Order, error) {
	...
	err := r.db.
		Preload("Table").
		Preload("Staff.Role").
		Preload("OrderItems.Menu.Category").
...
func (r *orderRepository) CreateOrderTx(order *models.Order) (*models.Order, error) {
	err := r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Clauses(clause.Locking{Strength: "UPDATE"}).First(&table, order.TablesID).Error; err != nil {
			return errors.New("meja tidak ditemukan")
		}
		// Jika meja masih available, set menjadi occupied otomatis.
		if table.Status == "available" {
			if err := tx.Model(&table).Update("status", "occupied").Error; err != nil {
				return err
			}
		} else if table.Status != "occupied" {
			// Untuk status lain (reserved, maintenance), tolak pembuatan order
			return errors.New("meja tidak tersedia untuk membuat order")
		}
	...
	r.db.Preload("OrderItems.Menu").Preload("Table").First(order, order.ID)
	return order, nil
}
...
func (r *orderRepository) AddItemsToOrderTx(order *models.Order, newItems []models.OrderItem) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		// Update field Total di tabel orders
...
func (r *orderRepository) MoveTableTx(order *models.Order, newTableID int) error {
	...
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(order).Update("tables_id", newTableID).Error; err != nil {
			return err
		}
...
func (r *orderRepository) SplitTableTx(oldOrder *models.Order, newOrder *models.Order, itemsToMove []models.OrderItem, itemsToUpdate []models.OrderItem, itemsToDelete []int) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		// Kunci & ubah meja tujuan menjadi "occupied"
		res := tx.Model(&models.Table{}).Where("id = ? AND status = ?", newOrder.TablesID, "available").Update("status", "occupied")
		if res.Error != nil {
			return res.Error
		}
		if res.RowsAffected == 0 {
		...
		if err := tx.Model(oldOrder).Update("total", oldOrder.Total).Error; err != nil {
			return err
		}
...
func (r *orderRepository) MergeOrdersTx(targetOrder *models.Order, sourceOrder *models.Order) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&models.Table{}).Where("id = ?", sourceOrder.TablesID).Update("status", "available").Error; err != nil {
			return err
		}

		// Update total uang di Target Order
		if err := tx.Model(targetOrder).Update("total", targetOrder.Total).Error; err != nil {
			return err
		}
...
func (r *orderRepository) UpdateOrderStatus(order *models.Order, status string) error {
	return r.db.Transaction(func(tx *gorm.DB) error {
