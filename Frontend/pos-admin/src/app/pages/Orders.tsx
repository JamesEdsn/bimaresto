...
const parseOrderDate = (createdAt: Date | string | undefined): Date => {
...
export default function Orders() {
  const { orders, orderItems, splitBills, updateOrderStatus } = useAppContext();
  
  // State for modal and editing
  const [selectedOrder, setSelectedOrder] = useState<Order | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editStatus, setEditStatus] = useState('');
  const [splitBillFilter, setSplitBillFilter] = useState('all'); // 'all', 'split', 'non_split'
  ...
  const hasSplitBill = (orderId: number) => {
    return Array.isArray(splitBills) && splitBills.some(sb => sb.order_id === orderId);
  };

  const getSplitBillsForOrder = (orderId: number) => {
    return Array.isArray(splitBills) ? splitBills.filter(sb => sb.order_id === orderId) : [];
  };

  const orderItemsForOrder = (orderId: number) => {
    return Array.isArray(orderItems) ? orderItems.filter(item => item.order_id === orderId) : [];
  };
  ...
  const filteredOrders = useMemo(() => {
    ...
    return [...orders]
      ...
      .filter(order => {
        ...
        if (splitBillFilter === 'split') {
          return hasSplitBill(order.id);
        } else if (splitBillFilter === 'non_split') {
          return !hasSplitBill(order.id);
        }
        return true;
      });
  }, [orders, startDate, endDate, splitBillFilter, splitBills]);
              ...
              <thead>
                <tr className="border-b border-slate-200">
                  ...
                  <th className="px-6 py-4 text-left">
                    <select
                      value={splitBillFilter}
                      onChange={(e) => setSplitBillFilter(e.target.value)}
                      className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 text-[12px] font-semibold text-slate-500 uppercase tracking-wider cursor-pointer outline-none focus:border-orange-500 transition-all focus:ring-1 focus:ring-orange-500/20"
                    >
                      <option value="all" className="normal-case font-normal text-slate-700 bg-white">Semua</option>
                      <option value="split" className="normal-case font-normal text-slate-700 bg-white">Split Bill</option>
                      <option value="non_split" className="normal-case font-normal text-slate-700 bg-white">Non Split Bill</option>
                    </select>
                  </th>
                  <th className="px-6 py-4 text-left text-[12px] font-semibold text-slate-500 uppercase tracking-wider">
                    Actions
                ...
                {filteredOrders.map((order, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-[14px] text-slate-500">
                      {parseOrderDate(order.created_at).toLocaleString('id-ID')}
                    </td>
                    <td className="px-6 py-4">
        ...
        <div className="fixed inset-0 bg-black/20 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-8 max-w-2xl w-full border border-slate-200 max-h-[90vh] overflow-y-auto shadow-xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div className="p-4 bg-slate-50 rounded-lg">
                <p className="text-slate-500 text-[12px] mb-1">Status</p>
                <p className="text-slate-950 text-[18px] font-bold capitalize">{selectedOrder.status}</p>
              </div>
              <div className="p-4 bg-slate-50 rounded-lg">
                <p className="text-slate-500 text-[12px] mb-1">Date & Time</p>
                <p className="text-slate-950 text-[18px] font-bold">{parseOrderDate(selectedOrder.created_at).toLocaleString('id-ID')}</p>
              </div>
            </div>
                  ...
                  {getSplitBillsForOrder(selectedOrder.id).map((split, index) => (
                    <div key={split.id} className="p-4 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-lg border border-indigo-200">
                      <div className="flex items-center justify-between mb-2">
                        <div>
                          <p className="text-slate-950 text-[14px] font-bold">
                            {split.person_name || `Split Bill #${split.id}`}
                          </p>
                        ...
                        <div className="text-right">
                          ...
                          <span className={`px-2 py-1 text-[10px] rounded-full capitalize ${
                            ...
                            {split.payment_status || 'split'}
                          </span>
                        </div>
                      </div>
                      <div className="flex items-center gap-4 text-[12px] text-slate-600 mt-2">
                        <span className="capitalize">
                          Method: {split.payment_method === 'e-wallet' ? 'E-Wallet' : split.payment_method === 'card' ? 'Card' : 'Split Bill'}
                        </span>
                        {split.paid_at && (
                          <span>Paid: {new Date(split.paid_at).toLocaleString('id-ID')}</span>
                        )}
                        <span>Created: {new Date(split.created_at).toLocaleString('id-ID')}</span>
                      </div>
                    </div>
                  ))}
