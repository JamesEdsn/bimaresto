...
export default function Dashboard() {
  ...
  // FALLBACK AMAN: Pastikan selalu berbentuk array [] meskipun data dari API kosong/undefined
  const orders = context?.orders || [];
  const tables = context?.tables || [];
  const menus = context?.menus || [];

  // Karena orderItems tidak ada di AppContext utama, kita ekstrak dari relasi orders
  const orderItems = (context as any)?.orderItems || orders.flatMap(order => order.order_items || []);
  ...
  // Get recent orders (last 4) - Diperbarui menggunakan new Date() untuk menghindari error string ISO API
          ...
          <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-lg">
            ...
            <div className="space-y-4 divide-y divide-slate-200">
              {recentOrders.length > 0 ? recentOrders.map((order) => (
                ...
                <p className="text-sm text-slate-500 py-4">Belum ada pesanan.</p>
              )}
            </div>
          </div>
            ...
            <div className="space-y-4 divide-y divide-slate-200">
              {topMenus.map((menu, index) => (
                <div key={menu.id} className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-200 shadow-md">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-bold shadow-sm flex-shrink-0 ${
                    index === 0
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-slate-900 text-[14px] font-medium mb-1 break-words">{menu.name}</p>
