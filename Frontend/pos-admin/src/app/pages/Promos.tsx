...
import { createPromo, deletePromoById, getPromos, updatePromoById } from '../../services/api';
...
export default function Promos() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingPromo) {
      const savedPromo = await updatePromoById({
        ...editingPromo,
        name: formData.name,
  ...
  const togglePromoStatus = async (id: number) => {
    ...
    const savedPromo = await updatePromoById({ ...target, is_active: !target.is_active });
    setPromos(promos.map(p => p.id === id ? savedPromo : p));
