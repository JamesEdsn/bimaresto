...
import { TableInfo, OrderItem, StationOrder, BuffetPackage, buffetPackages, categories as defaultCategories } from "../../data/posMenuData";
...
export function TableDetail({
  ...
  const totalSent = Object.values(table.sentItems).reduce((s: number, v: any) => s + v, 0);
  const hasSentItems = totalSent > 0;
  const hasUnsentItems = orders.some((o: OrderItem) => o.quantity - (table.sentItems[o.id] || 0) > 0);
  const unsentCount = orders.reduce((s: number, o: OrderItem) => s + Math.max(0, o.quantity - (table.sentItems[o.id] || 0)), 0);
  ...
  const filteredMenu = useMemo(() => {
    return menuPackages.filter((item: BuffetPackage) => {
  ...
  const removeItem = (id: string) => {
    ...
    onUpdateOrders(
      table.id,
      orders.filter((o: OrderItem) => o.id !== id)
    );
  };
            ...
            <div className="bg-white rounded-2xl w-full max-w-md border border-gray-200 overflow-hidden">
              ...
              <div className="p-6 flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 py-3 rounded-xl bg-orange-500 text-white transition flex items-center justify-center gap-2 text-sm"
                >
      ...
      <div className="h-full flex flex-col bg-gray-50">
        ...
        <div className="flex-1 overflow-y-auto p-4 lg:p-6">
          <div className="max-w-lg mx-auto space-y-5">
            ...
            <button
              onClick={handlePay}
              className="w-full bg-orange-500 text-white py-4 rounded-xl transition flex items-center justify-center gap-2"
            >
        ...
        <div className="flex items-center gap-2">
          ...
          {orders.length > 0 && (
            <div className="relative" ref={actionsRef}>
              ...
              {showActions && (
                <div className="absolute right-0 top-11 w-64 bg-white rounded-xl border border-gray-200 py-2 z-40">
