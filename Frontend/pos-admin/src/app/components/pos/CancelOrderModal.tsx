...
export function CancelOrderModal({ table, onCancelItems, onCancelAll, onClose }: CancelOrderModalProps) {
  ...
  const selectedTotal = orders
    .filter((o: OrderItem) => selectedItems.has(o.id))
    .reduce((s: number, o: OrderItem) => s + o.price * o.quantity, 0);
  ...
  // ======== CONFIRM STEP ========
  if (mode === "confirm") {
    const cancelledItems = cancelType === "all" ? orders : orders.filter((o: OrderItem) => selectedItems.has(o.id));
    const cancelTotal = cancelledItems.reduce((s: number, o: OrderItem) => s + o.price * o.quantity, 0);
        ...
        <div className="bg-white rounded-2xl max-w-md w-full max-h-[85vh] overflow-hidden flex flex-col" onClick={(e: React.MouseEvent<HTMLDivElement>) => e.stopPropagation()}>
          ...
          <div className="p-5 overflow-y-auto flex-1 space-y-4">
            ...
            <div className="space-y-1">
              ...
              {cancelledItems.map((o: OrderItem) => (
        ...
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-gray-400 text-xs">{table.name} &bull; {orders.reduce((s: number, o: OrderItem) => s + o.quantity, 0)} item</p>
            </div>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-lg hover:bg-gray-100 flex items-center justify-center transition">
            <X className="w-4 h-4 text-gray-400" />
