...
import { TableInfo } from "../../data/posMenuData";
...
export function TableGrid({ tables, onSelectTable }: TableGridProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
      {tables.map((table) => {
        const totalOrderItems = table.orders.reduce((s: number, o) => s + o.quantity, 0);
        const totalPrice = table.orders.reduce((s: number, o) => s + o.price * o.quantity, 0);
        ...
        const hasUnsent = table.orders.some(
          (o: any) => o.quantity - (table.sentItems[o.id] || 0) > 0
        );
        ...
        return (
          <button
            ...
            className={`relative p-5 rounded-2xl border-2 transition-colors text-left group ${
              hasSent
          ...
          >
            {/* Station sent badge */}
            {hasSent && (
              <div className="absolute -top-2 -right-2 bg-rose-500 text-white px-2 py-0.5 rounded-full text-[10px] flex items-center gap-1 border border-rose-600">
            ...
            {hasSent && hasUnsent && (
              <div className="absolute -top-2 left-2 bg-amber-500 text-white px-2 py-0.5 rounded-full text-[10px] flex items-center gap-1 border border-amber-600">
