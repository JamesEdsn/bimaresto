...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const handleStorage = (event: StorageEvent) => {
      if (event.key === SHARED_DATA_UPDATE_KEY) {
        refreshData();
      }
      syncAuthState();
    };
  ...
  return (
    ...
    >
      {isInitialLoad && isLoading ? (
        ...
        <>
          {children}
          {/* Global manual refresh button (appears on all pages) */}
          <div className="fixed right-4 bottom-6 z-50">
            <button
              onClick={() => refreshData()}
              disabled={isLoading}
              title="Refresh data"
              className="inline-flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg shadow-sm text-sm text-slate-700 hover:bg-slate-50 disabled:opacity-60"
            >
              Refresh
            </button>
          </div>
